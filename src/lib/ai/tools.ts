...
export const toolDefinitions: Anthropic.Tool[] = [
  {
    ...
    description:
      "Query attendance records for a date range. Returns summary counts by status and individual records (capped at 100), including a free-text `note` field per record (remarks, injury details, explanations, etc. — may be null), payment fields `paymentType` (BONUS, EZAM_ELAVE, or AVANS — null if none), `paymentAmount` (calculated ₼ value, null if none), and `paymentPaid` (₼ actually paid so far — may be less than paymentAmount for a partial payment, null if no payment type), and company-expense fields `expenseType` (FOOD, TOOL, FINE, or OTHER — null if none) with `expenseAmount` (₼ the company spent via this employee — e.g. money given for food/tools, or a traffic fine paid on their behalf — null if none; this is company spending, not employee debt, unlike AVANS). Use for questions about who worked, absences, trip counts, car usage, weekend/holiday work, notes/remarks, bonus/travel-allowance/advance payments and whether they're paid, or company spending/fines routed through employees.",
    input_schema: {
      type: "object",
      properties: {
